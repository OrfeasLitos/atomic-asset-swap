...
static void init_buf_state(struct buffer_state * state, const void * input, size_t len) obliv
{
  state->p = input;
  state->len = len;
  state->total_len = len;
...
/* Return value: bool */
static int calc_chunk(uint8_t chunk[CHUNK_SIZE], struct buffer_state * state) obliv
{
 ...
 */
void calc_sha_256(obliv uint8_t hash[32], const obliv uint8_t *input, size_t len) obliv
{
  /*
...
void hash_to_string(char string[65], const obliv uint8_t hash[32])
{
  size_t i;
  for (i = 0; i < 32; i++) {
    uint8_t j;
